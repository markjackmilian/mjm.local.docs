...
**Note on the Database provider.** With `FileStorage: Database` the content lives in the row and `FileStorageLocation` is null for every document, so the new read returns nothing and the check is free. It matters for `FileSystem` and `AzureBlob`. Correction to an earlier claim in this plan’s predecessors: this machine’s user secrets select `FileStorage: Database`, not `AzureBlob` — the AzureBlob *settings* are present but the provider is not selected. So the missing-file check is currently a no-op here, and the file leak the predecessor plans fixed is currently latent rather than active. The embedding leak was and is active, because `Storage` is `SqlServer` and `chunk_embeddings` is still outside EF. This check earns its place the moment the provider changes, and the settings being present suggests it will.
